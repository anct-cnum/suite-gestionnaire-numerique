...
export default function FiltreCommune({ communes, communesSelectionnees }: Props): ReactElement {
  ...
  const [pending, setPending] = useState<ReadonlyArray<SelectOption>>(
    communesSelectionnees.map(({ codeInsee, nom }) => ({ label: nom, value: codeInsee }))
  )

  const communesKey = communes.join(',')
  useEffect(() => {
    setPending(communesSelectionnees.map(({ codeInsee, nom }) => ({ label: nom, value: codeInsee })))
  }, [communesKey])

  const isFilled = pending.length > 0
  const labelBouton = isFilled ? `Commune · ${pending.length}` : 'Communes'

  const appliquer = useCallback(
    (selection: ReadonlyArray<SelectOption>) => {
      const params = new URLSearchParams(searchParams.toString())
      if (selection.length > 0) {
        params.set('communes', selection.map((opt) => opt.value).join(','))
      } else {
  ...
  const onInputChange = useCallback(
    (event: ChangeEvent<HTMLInputElement>) => {
      ...
      timerRef.current = setTimeout(() => {
        void fetch(`/api/statistiques/communes?q=${encodeURIComponent(val)}`)
          .then((res) => res.json())
          .then((data: ReadonlyArray<CommuneOption>) => {
            setSearchResults(data.map(({ codeInsee, nom }) => ({ label: nom, value: codeInsee })))
          })
      }, 300)
    },
      ...
      {isOpen ? (
        <>
          ...
          <div className={styles.popover}>
            ...
            >
              <label className="fr-label fr-mb-1v fr-text--bold" htmlFor="commune-filter-select">
                Filtrer par&nbsp;:
              </label>
              <input
                autoComplete="off"
                className="fr-input fr-mb-2v fr-mt-3v"
                id="commune-filter-select"
                list="commune-filter-datalist"
                onChange={onInputChange}
                placeholder="Chercher une commune"
                value={inputValue}
              />
              <datalist id="commune-filter-datalist">
                {searchResults
              ...
              {isFilled ? (
                <>
                  ...
                  <span className="fr-text--bold fr-text--sm">
                    {pending.length}&nbsp;{pending.length > 1 ? 'communes sélectionnées' : 'commune sélectionnée'}
                  </span>
