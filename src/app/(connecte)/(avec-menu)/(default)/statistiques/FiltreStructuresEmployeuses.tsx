...
export default function FiltreStructuresEmployeuses({ structures, structuresSelectionnees }: Props): ReactElement {
  ...
  const [pending, setPending] = useState<ReadonlyArray<SelectOption>>(
    structuresSelectionnees.map(({ id, nom }) => ({ label: nom, value: id }))
  )

  const structuresKey = structures.join(',')
  useEffect(() => {
    setPending(structuresSelectionnees.map(({ id, nom }) => ({ label: nom, value: id })))
  }, [structuresKey])

  const isFilled = pending.length > 0
  const labelBouton = isFilled ? `Structure · ${pending.length}` : 'Structures employeuses'

  const appliquer = useCallback(
    (selection: ReadonlyArray<SelectOption>) => {
      const params = new URLSearchParams(searchParams.toString())
      if (selection.length > 0) {
        params.set('structuresEmployeuses', selection.map((opt) => opt.value).join(','))
      } else {
  ...
  const onInputChange = useCallback(
    (event: ChangeEvent<HTMLInputElement>) => {
      ...
      timerRef.current = setTimeout(() => {
        void fetch(`/api/statistiques/structures-employeuses?q=${encodeURIComponent(val)}`)
          .then((res) => res.json())
          .then((data: ReadonlyArray<StructureEmployeuseOption>) => {
      ...
      {isOpen ? (
        <>
          ...
          <div className={styles.popover}>
            ...
            >
              <label className="fr-label fr-mb-1v fr-text--bold" htmlFor="structure-employeuse-filter-select">
                Filtrer par&nbsp;:
              </label>
              <input
                autoComplete="off"
                className="fr-input fr-mb-2v fr-mt-3v"
                id="structure-employeuse-filter-select"
                list="structure-employeuse-filter-datalist"
                onChange={onInputChange}
                placeholder="Chercher une structure employeuse"
                value={inputValue}
              />
              <datalist id="structure-employeuse-filter-datalist">
                {searchResults
              ...
              {isFilled ? (
                <>
                  ...
                  <span className="fr-text--bold fr-text--sm">
                    {pending.length}&nbsp;{pending.length > 1 ? 'structures sélectionnées' : 'structure sélectionnée'}
                  </span>
...
type Props = Readonly<{
  structures: ReadonlyArray<string>
  structuresSelectionnees: ReadonlyArray<StructureEmployeuseOption>
}>
