...
describe('gouvernance', () => {
  ...
  it('quand j’affiche une gouvernance avec au moins un membre, alors elle s’affiche avec son résumé et sa section membre', () => {
    ...
    const MembreOne = within(columns1Body[1]).getByRole('button', { name: 'Préfecture du Rhône' })
    expect(MembreOne).toHaveAttribute('type', 'button')
    expect(MembreOne).toHaveAttribute('aria-controls', 'draweMembreId')
    ...
    const MembreTwo = within(columns2Body[1]).getByRole('button', { name: 'Département du Rhône' })
    expect(MembreTwo).toHaveAttribute('type', 'button')
    expect(MembreTwo).toHaveAttribute('aria-controls', 'draweMembreId')
