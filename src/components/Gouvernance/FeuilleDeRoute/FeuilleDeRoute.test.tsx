...
describe('feuille de route', () => {
  it('quand je clique sur une feuille de route, alors un drawer s’ouvre avec les détails de la feuille de route', () => {
    ...
    const montantDeLaSubventionDemandeeLabel = within(drawer).getByText('Montant des subventions d\'ingenierie demandées')
    expect(montantDeLaSubventionDemandeeLabel).toBeInTheDocument()
    const montantDeLaSubventionDemandee = within(drawer).getByText('145 000 €')
    expect(montantDeLaSubventionDemandee).toBeInTheDocument()
    const montantDeLaSubventionAccordeeLabel = within(drawer).getByText('Montant des subventions d\'ingenierie accordées')
    expect(montantDeLaSubventionAccordeeLabel).toBeInTheDocument()
    const montantDeLaSubventionAccordee = within(drawer).getByText('145 000 €')
    expect(montantDeLaSubventionAccordee).toBeInTheDocument()
    const beneficiairesDesSubventionsLabel = within(drawer).getByText('Bénéficiaire des subventions d\'ingenierie accordées')
    ...
    const beneficiaireDesSubventionsFormationLabel = within(drawer).getByText('Bénéficiaire des subventions d\'ingenierie accordées')
  ...
  it('quand je suis dans le détail d’une feuille de route, s’il n’y a pas de bénéficiaire de subvention alors un tiret est affiché à la place de la liste des bénéficiaires et les labels sont au singulier', () => {
    ...
    const beneficiairesDesSubventionsLabel = within(drawer).getByText('Bénéficiaire des subventions d\'ingenierie accordées')
    expect(beneficiairesDesSubventionsLabel).toBeInTheDocument()
    const beneficiaireDesSubventionsFormationLabel = within(drawer).getByText('Bénéficiaire de la subvention formation accordée')
    expect(beneficiaireDesSubventionsFormationLabel).toBeInTheDocument()
