...
export default function FeuilleDeRouteRemplie({ feuillesDeRoute }: Props): ReactElement {
  // Stryker disable next-line BooleanLiteral
  return (
    <Table enTetes={['Logo', 'Nom', 'Action', 'Budget total']} isHeadHidden={true} titre="Feuilles de route">
      {feuillesDeRoute.map((feuilleDeRoute) => (
        <tr key={feuilleDeRoute.nom}>
          ...
          <td className="color-grey">{feuilleDeRoute.totalActions}</td>
          <td className="font-weight-700">{feuilleDeRoute.budgetGlobal}</td>
        </tr>
      ))}
    </Table>
  )
