  ...
  return (
    <section aria-labelledby="role" className="grey-border border-radius fr-mb-2w fr-p-4w">
      ...
      {role.gouvernances.length > 0 ? (
        <Table enTetes={['Gouvernance', 'Rôle dans la gouvernance']} titre="Rôle de la structure dans les gouvernances">
          {role.gouvernances
            .toSorted((gouvernanceA, gouvernanceB) => gouvernanceA.code.localeCompare(gouvernanceB.code))
            .map((gouvernance) => (
              <tr key={gouvernance.code}>
                <td>
                  <Link href={`/gouvernance/${gouvernance.code}`}>
                    <strong>
                      {gouvernance.nom} ({gouvernance.code})
                    </strong>
                  </Link>
                </td>
                <td>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
                    {gouvernance.roles.map((roleItem) => (
                      <Badge color={roleItem.color} key={roleItem.nom}>
                        {roleItem.nom}
                      </Badge>
                    ))}
                  </div>
        ...
        <p className="color-grey">Aucune gouvernance</p>
      )}
    </section>
  )
}
