...
describe('formulaire d‘ajout d‘une action', () => {
  describe('menu latéral', () => {
    it.each([
      ...
      'étant un utilisateur, lorsque je veux ajouter une action, alors je vois le menu latéral avec le lien %s',
      ({ lien, titre }) => {
        // WHEN
        afficherMenuLateral()
  ...
  describe('formulaire', () => {
    it('étant un utilisateur,lorsque je veux ajouter une action, alors je vois le formulaire d‘ajout d‘une action', () => {
      ...
      const titre = within(formulaire).getByRole('heading', {
        level: 1,
        name: 'Ajouter une action à la feuille de route',
      })
      expect(titre).toBeInTheDocument()
      const titreIndicationDuBesoin = within(formulaire).getByText(matchWithoutMarkup('Besoins liés à l‘action *'), {
        selector: 'p',
      })
      expect(titreIndicationDuBesoin).toBeInTheDocument()
      const boutonAjouter = within(formulaire).getByRole('button', {
        description: 'Ajouter des besoins',
        name: 'Ajouter',
      })
      ...
      const titreSectionInformationSurLAction = within(formulaire).getByText('Informations sur l‘action', {
        selector: 'p',
      })
      ...
      const labelContexteAction = within(formulaire).getAllByText(
      ...
      const labelDescriptionAction = within(formulaire).getAllByText(
        'Préciser la nature de l‘action, ses objectifs, ses bénéficiaires, son impact et indicateurs associés.'
      )[1]
      ...
      const labelSectionPorteurDeLAaction = within(formulaire).getByText(
        'Précisez la ou les structure(s) porteuse(s) de cette action',
        { selector: 'p' }
      )
      expect(labelSectionPorteurDeLAaction).toBeInTheDocument()
      const boutonAjouterLePorteurDeLAaction = within(formulaire).getByRole('button', {
        description: 'Ajouter des porteurs',
        name: 'Ajouter',
      })
      expect(boutonAjouterLePorteurDeLAaction).toBeInTheDocument()
      const titreSectionTemporaliteDeLAaction = within(formulaire).getByText('Temporalité de l‘action', {
        selector: 'p',
      })
      expect(titreSectionTemporaliteDeLAaction).toBeInTheDocument()
      const labelSectionTemporaliteDeLAaction = within(formulaire).getByText(
      ...
      const instructionsInformationBudget = within(formulaire).getByText(
        'Détaillez le budget prévisionnel de l‘action incluant les subventions et les co-financements éventuels des membres ou ...',
        { selector: 'p' }
      )
    ...
    it('étant un utilisateur, lorsque j‘ouvre le formulaire de modification d‘une action, alors je vois le contenu de l‘action', () => {
      ...
      afficherFormulaireDeModificationAction(
        ...
        },
        {
          ...
          destinataires: [
            ...
            {
              id: 'membreFooId2',
              link: '/gouvernance/69/membre/membreFooId2',
      ...
      const boutonModifierLesBesoins = within(formulaire).getByRole('button', {
        description: 'Modifier les besoins',
        name: 'Modifier',
      })
    ...
    it('étant un utilisateur, lorsque je modifie une action, alors je peux la valider', async () => {
      ...
      await waitFor(() => {
        expect(modifierUneActionAction).toHaveBeenCalledWith({
          anneeDeDebut: '2026',
          anneeDeFin: '2028',
          besoins: ['co_construire_la_feuille_de_route', 'monter_dossiers_de_subvention'],
          budgetGlobal: 1000,
          coFinancements: [
            {
              coFinanceur: 'Cofinanceur 1',
    ...
    it('étant un utilisateur, lorsque je remplis correctement le formulaire, alors je peux voir les différents états du bouton', () => {
      ...
      const ajouterUneActionAction = stubbedServerAction(['OK'])
      renderComponent(
        <AjouterUneAction action={actionViewModelFactory()} date={epochTime} uidFeuilleDeRoute="uidFeuilleDeRoute" />,
    ...
    it('étant un utilisateur, quand je renseigne un besoin et que je enregistre alors le drawer se ferme et le(s) besoin(s) s’affiche', () => {
      ...
      const deuxiemeBesoin = within(formulaire).getByText(
        'Appuyer la certification Qualiopi de structures privées portant des formations à l’inclusion numérique',
        { selector: 'p' }
      )
      expect(deuxiemeBesoin).toBeInTheDocument()
      const troisiemeBesoin = within(formulaire).getByText('Structurer une filière de reconditionnement locale', {
        selector: 'p',
      })
      expect(troisiemeBesoin).toBeInTheDocument()
      const quatriemebesoin = within(formulaire).getByText('Monter des dossiers de subvention complexes', {
        selector: 'p',
      })
      expect(quatriemebesoin).toBeInTheDocument()
      const besoinNonSelectionnerOne = within(formulaire).queryByText(
        'Structurer un fond local pour l’inclusion numérique',
        { selector: 'p' }
      )
...
export function afficherFormulaireDeCreationAction(
  ...
  renderComponent(
    <AjouterUneAction action={actionVideViewModelFactory()} date={epochTime} uidFeuilleDeRoute="uidFeuilleDeRoute" />,
    options,
...
function afficherFormulaireDeCreationValidation(
  ...
  renderComponent(
    ...
    >
      <SubmitButton isDisabled={false}>Valider et envoyer</SubmitButton>
    </FormulaireAction>,
    { ajouterUneActionAction }
  )
}
