...
describe('formulaire d‘ajout d‘une action', () => {
  ...
  describe('formulaire', () => {
    ...
    it('étant un utilisateur, lorsque j‘ouvre le formulaire de modification d‘une action, alors je vois le contenu de l‘action', () => {
      ...
      expect(listeCofinancements).toHaveLength(2)
      ...
      const montantPremierCofinancement = within(listeCofinancements[0]).getByText('5 000 €')
      expect(montantPremierCofinancement).toBeInTheDocument()
      const deuxiemeCofinancement = within(listeCofinancements[1]).getByText('Croix Rouge Française')
      expect(deuxiemeCofinancement).toBeInTheDocument()
      const montantDeuxiemeCofinancement = within(listeCofinancements[1]).getByText('5 000 €')
      expect(montantDeuxiemeCofinancement).toBeInTheDocument()
      const boutonSupprimerCofinancement = within(listeCofinancements[1]).getByRole('button', { name: 'Supprimer' })
