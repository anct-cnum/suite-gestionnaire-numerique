...
describe('faire une demande de subvention ', () => {
  describe('étant donné que je me trouve sur le formulaire d’action', () => {
    it('et que je n’ai pas saisi de budget global, alors le bouton de demande de subvention est inactif', () => {
      ...
      // WHEN
      afficherFormulaireAction(budgetGlobal)

      // THEN
      const boutonDemandeDeSubvention = screen.getByRole(
        'button', { description: 'Faire une demande de subvention', name: 'Demander une subvention' }
      )
    ...
    it('et que j’ai saisi un budget global valide, quand je clique sur "Demander une subvention", alors le'
      + ' sous-formulaire de demande de subvention s’affiche sans enveloppe sélectionnée, indiquant le maximum autorisé'
      + ' pour cette action correspondant au budget global préalablement saisi, avec des champs de saisie de montants et'
      + ' un bouton de soumission désactivés', () => {
      // GIVEN
      afficherFormulaireAction(42_500)

      // WHEN
      ouvrirFormulaireDemandeDeSubvention()

      // THEN
      const drawer = sousFormulaireDemandeSubvention()
      const titre = within(drawer).getByRole('heading', { level: 1, name: 'Demander une subvention' })
      ...
      const selecteurEnveloppes = within(drawer).getByRole('combobox', { name: 'Enveloppe de financement concernée' })
      const enveloppes = within(selecteurEnveloppes).getAllByRole('option')
      expect(selecteurEnveloppes).toBeInTheDocument()
      expect(enveloppes).toHaveLength(5)
      expect(enveloppes[0].textContent).toBe('Choisir')
      expect(enveloppes[0]).toBeDisabled()
      ...
      const descriptionMontantRh = within(drawer)
        .getByText(
          matchWithoutMarkup(
            'Il s’agit d’une ressource humaine interne à la structure employeuse faisant partie de la gouvernance et'
            + ' récipiendaire des fonds. Format attendu : Montant ETP en euros'
          ),
          { selector: 'p' }
        )
      const abbreviationEtp = within(descriptionMontantRh).getByText('ETP', { selector: 'abbr' })
      ...
      expect(descriptionMontantRh).toBeInTheDocument()
  ...
  describe('étant donné que je me trouve sur le sous-formulaire de demande de subvention', () => {
    it('quand je clique sur le bouton de fermeture, alors le sous-formulaire se ferme', () => {
      ...
      // WHEN
      const drawer = sousFormulaireDemandeSubvention()
      const boutonFermer = within(drawer).getByRole('button', { name: 'Fermer la demande de subvention' })
    ...
    describe('quand je sélectionne une enveloppe de financement', () => {
      it.each([
        ...
        + ' saisie de montants deviennent actifs', ({ enveloppeId, expectedLimiteAffichee, expectedDroitsDeSubventionAffiches }) => {
        // GIVEN
        afficherFormulaireAction(42_500)
        ouvrirFormulaireDemandeDeSubvention()

        // WHEN
        selectionnerEnveloppe(enveloppeId)
    ...
    describe('et que j’ai sélectionné une enveloppe de financement, quand je saisis des montants', () => {
      describe('dont la somme excède le montant de l’enveloppe ou le budget maximal de l’action, puis que je corrige'
        ...
        ])('$desc', ({ montantPrestationInitial, montantPrestationRectifie, montantRhInitial, montantRhRectifie }) => {
          // GIVEN
          afficherFormulaireAction(42_500)
          ouvrirFormulaireDemandeDeSubvention()
          selectionnerEnveloppe('4')
          saisirMontants(montantPrestationInitial, montantRhInitial)

          // WHEN
          saisirMontants(montantPrestationRectifie, montantRhRectifie)
      ...
      describe('dont la somme excède le montant de l’enveloppe qui est inférieur au budget maximal de l’action, alors'
        + ' je ne peux soumettre ma demande', () => {
        it.each([
          {
          ...
          },
          {
            ...
            montantRh: 2,
            raison: 'car l’un des montants est saisi correctement mais l’autre résulte en une somme qui excède celui'
              + ' de l’enveloppe',
          },
          {
            ...
            enveloppeInitialeId: '3',
            expectedLimiteAffichee: '10 000',
            expectedTotalAffiche: '30 000',
            montantPrestation: 15_000,
            montantRh: 15_000,
            raison: 'CHANGEMENT de l’enveloppe',
          },
        ])('$raison', ({
          enveloppeInitialeId,
          ...
          afficherFormulaireAction(42_500)
          ouvrirFormulaireDemandeDeSubvention()
          selectionnerEnveloppe(enveloppeInitialeId)

          // WHEN
          saisirMontants(montantPrestation, montantRh)
          selectionnerEnveloppe(enveloppeDeRemplacementId)
      ...
      describe('dont la somme excède le budget maximal de l’action qui est inférieur au montant de l’enveloppe,'
        + ' alors je ne peux soumettre ma demande', () => {
        it.each([
          {
            montantPrestation: 0,
            montantRh: 42_501,
          ...
          },
          {
            montantPrestation: 2,
            montantRh: 42_499,
            raison: 'car l’un des montants est saisi correctement mais l’autre résulte en une somme qui excède'
              + ' le budget maximal de l’action',
          },
        ])('$raison', ({ montantPrestation, montantRh }) => {
          // GIVEN
          afficherFormulaireAction(42_500)
          ouvrirFormulaireDemandeDeSubvention()
          selectionnerEnveloppe('2')
      ...
      describe('dont la somme n’excède ni le montant de l’enveloppe, ni celui du budget total de l’action', () => {
        describe('alors la somme s’affiche en tant que total des subventions demandées et je peux soumettre ma'
        ...
        it('et que je soumets ma demande, alors le sous-formulaire se ferme', () => {
          ...
          afficherFormulaireAction(42_500)
          ouvrirFormulaireDemandeDeSubvention()
          const drawer = screen.getByRole('dialog', { hidden: false, name: 'Demander une subvention' })
          selectionnerEnveloppe('4')
          saisirMontants(800, 3_300)
...
function afficherFormulaireAction(budgetGlobal: number): void {
  renderComponent(
    <FormulaireAction
...
function ouvrirFormulaireDemandeDeSubvention(): void {
  presserLeBouton('Demander une subvention', 'Faire une demande de subvention')
}

function selectionnerEnveloppe(enveloppeId: string): void {
  const selecteurEnveloppes = within(sousFormulaireDemandeSubvention())
    .getByRole('combobox', { name: 'Enveloppe de financement concernée' })
  fireEvent.change(selecteurEnveloppes, { target: { value: enveloppeId } })
}

function saisirMontants(montantPrestation: number, montantRh: number): void {
  const [champSaisieMontantPrestation, champSaisieMontantRh] = champsSaisieMontants()
...
function champsSaisieMontants(): Readonly<[HTMLHtmlElement, HTMLElement]> {
  ...
  return [
    ...
    within(sousFormulaire).getByRole('spinbutton', { name: 'Montant en ressources humaines' }),
  ]
}
