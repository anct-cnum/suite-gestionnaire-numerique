...
export function AlertePrevisionnel({ budgetGlobal, cofinancements, demandeDeSubvention }: Props): null | ReactElement {
  ...
  const montantCofinancements = cofinancements.reduce(
    (acc, cofinancement) => acc + (Number(cofinancement.montant) || 0),
    0
  )
  ...
  return (
    <div className="fr-p-2w background-red space-between">
      <div className="fr-grid-row space-between">
        <p className="fr-col-10 fr-mb-0 fr-text--bold color-red-strong">Total prévisionnel</p>
        <p className="fr-mb-0 fr-mr-2w color-red-strong fr-text--lg fr-text--bold">{formatMontant(-bilan)}</p>
      </div>
    </div>
  )
}
