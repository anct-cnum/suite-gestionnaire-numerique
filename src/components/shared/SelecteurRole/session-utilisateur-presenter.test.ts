...
describe(
  `Affichage des informations de session de l'utilisateur connecté ${utilisateur.prenom}` +
  ...
  () => {
    it.each([
      ...
      '$role : $expected.role.libelle avec le pictogramme $expected.role.pictogramme',
      ({ role, expected }) => {
        expect(
          sessionUtilisateurPresenter(makeUtilisateur(new Role({ typologie: role })))
      ...
      ({ role, perimetre, expected }) => {
        expect(
          sessionUtilisateurPresenter(
            makeUtilisateur(new Role({ territoireOuStructure: perimetre, typologie: role }))
          )
        ).toStrictEqual(expected)
      }
    )
