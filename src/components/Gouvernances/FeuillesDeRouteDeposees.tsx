...
export default function FeuillesDeRouteDeposees({ dateGeneration, feuillesDeRouteDeposees }: Props): ReactElement {
  ...
  if (isErrorViewModel(feuillesDeRouteDeposees)) {
    return (
      <section aria-labelledby="feuilles-de-route-deposees" className="fr-mb-4w grey-border border-radius fr-p-4w">
        <div style={{ alignItems: 'center', display: 'flex', justifyContent: 'center', padding: '2rem' }}>
          <div style={{ textAlign: 'center' }}>
            ...
            <div className="fr-text--sm color-blue-france fr-mt-2w">{feuillesDeRouteDeposees.message}</div>
          </div>
        </div>
      </section>
    )
  ...
  const allItems = feuillesDeRouteDeposees.ventilationParPerimetre.filter((item) => item.perimetre !== 'Autre')
  ...
  return (
    <section aria-labelledby="feuilles-de-route-deposees" className="fr-mb-4w grey-border border-radius fr-p-4w">
      <div className="center" ref={componentRef}>
        <div>
          <Doughnut
        ...
        <div className="fr-text--lg font-weight-700 fr-m-0">Feuilles de route</div>
        ...
        <div className="fr-mt-4w">
          <ul>
            {allItems.map((item) => (
              <li className="fr-grid-row fr-btns-group--space-between fr-mb-1w" key={item.perimetre}>
                <div className={styles['text-ellipsis']} style={{ flex: 1, minWidth: 0 }}>
                  <span className={styles['dot-margin']}>
                    <Dot color={item.color} />
                  </span>
                  <span className={styles['item-type-padding']}>{item.perimetre}</span>
                </div>
      ...
      <div className="fr-grid-row fr-grid-row--middle fr-mt-2w" style={{ alignItems: 'center' }}>
        <div style={{ flex: 1 }}>
