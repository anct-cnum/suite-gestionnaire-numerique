...
export default function GouvernancesTerritoriales({ dateGeneration, gouvernancesTerritoriales }: Props): ReactElement {
  ...
  if (isErrorViewModel(gouvernancesTerritoriales)) {
    return (
      <section aria-labelledby="gouvernances-territoriales" className="fr-mb-4w grey-border border-radius fr-p-4w">
        <div style={{ alignItems: 'center', display: 'flex', justifyContent: 'center', padding: '2rem' }}>
          <div style={{ textAlign: 'center' }}>
            ...
            <div className="fr-text--sm color-blue-france fr-mt-2w">{gouvernancesTerritoriales.message}</div>
          </div>
        </div>
      </section>
    )
  ...
  return (
    <section aria-labelledby="gouvernances-territoriales" className="fr-mb-4w grey-border border-radius fr-p-4w">
      <div className="center" ref={componentRef}>
        ...
        <div className="fr-text--lg font-weight-700 fr-m-0">Gouvernances territoriales</div>
        <div className="color-blue-france fr-pb-4w separator">
          dont{' '}
          <strong>
            {gouvernancesTerritoriales.nombreTotal - gouvernancesTerritoriales.sansCoporteur.count} co-portées
          </strong>
        </div>
        <div className="fr-mt-4w">
          <ul>
            {allItems.map((item) => (
              <li className="fr-grid-row fr-btns-group--space-between fr-mb-1w" key={item.type}>
                <div className={styles['text-ellipsis']} style={{ flex: 1, minWidth: 0 }}>
                  <span className={styles['dot-margin']}>
                    <Dot color={item.color} />
                  </span>
                  <span className={styles['item-type-padding']}>{item.type}</span>
                </div>
