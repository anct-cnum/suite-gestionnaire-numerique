...
describe('mes informations personnelles', () => {
  it('étant connecté quand j’affiche mes informations personnelles alors elles s’affichent', () => {
    ...
    const sousTitre = screen.getByText('Retrouvez ici, les informations de votre compte.', {
      selector: 'p',
    })
    expect(sousTitre).toBeInTheDocument()

    const mesInformationsPersonnelles = screen.getByRole('region', {
    ...
    const modifierMesInfosPersos = within(mesInformationsPersonnelles).getByRole('button', {
      name: 'Modifier',
    })
    ...
    const lienContacterLeSupport = within(sousTitreMonRole).getByRole('link', {
      name: 'Contacter le support',
    })
    expect(lienContacterLeSupport).toHaveAttribute(
      'href',
      'https://aide.conseiller-numerique.gouv.fr/fr/'
    )
  ...
  it.each([
    ...
    'étant un $role quand j’affiche mes informations personnelles alors l’encart "structure" ne s’affiche pas',
    ({ role }) => {
      // GIVEN
  ...
  it.each([
    ...
    'étant un $role quand j’affiche mes informations personnelles alors l’encart "structure" s’affiche',
    ({ role }) => {
      // GIVEN
      const presenter = mesInformationsPersonnellesPresenter({
        ...mesInformationsPersonnellesDTO,
      ...
      const titreMaStructure = within(maStructure).getByRole('heading', {
        level: 2,
        name: 'Ma structure',
      })
      ...
      const numeroDeSiretLabel = within(maStructure).getByText(
        matchWithoutMarkup('Numéro de SIRET')
      )
      ...
      expect(abreviationSiret).toHaveAttribute(
        'title',
