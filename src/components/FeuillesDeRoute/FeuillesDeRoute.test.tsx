...
describe('les feuilles de route d’une gouvernance', () => {
  it('quand j’affiche la page des feuilles de route, alors elles s’affichent avec leurs actions s’il y’en a et le total des budgets', () => {
    ...
    afficherLesFeuillesDeRoute(
      {},
      feuillesDeRouteReadModelFactory({
        feuillesDeRoute: [
          {
    ...
    const labelMontantTotalDesSubventions = within(sectionBudgetGlobal).getByText('Total des financements accordés', {
      selector: 'p',
    })
    ...
    const labelMontantTotalDesCofinancements = within(sectionBudgetGlobal).getByText('Total des co-financements', {
      selector: 'p',
    })
    ...
    const labelBudgetTotalDesFeuillesDeRoute = within(sectionBudgetGlobal).getByText(
    ...
    const titreDeLaPremiereFeuilleDeRoute = within(premiereFeuilleDeRoute).getByRole('heading', {
      level: 2,
      name: 'Feuille de route 1',
    })
    ...
    expect(lientVoirDetails).toHaveAttribute(
      'href',
      '/gouvernance/gouvernanceFooId/feuille-de-route/feuilleDeRouteFooId1'
    )
    const lienPorteurDeLaFeuilleDeRoute = within(premiereFeuilleDeRoute).getAllByRole('link', {
      name: 'CC des Monts du Lyonnais',
    })[0]
    expect(lienPorteurDeLaFeuilleDeRoute).toHaveAttribute('href', '/structure/200')
    const wordingNombreCofinanceursEtBeneficiaires = within(premiereFeuilleDeRoute).getByText(
      matchWithoutMarkup('5 bénéficiaires, 3 co-financeurs')
    )
    expect(wordingNombreCofinanceursEtBeneficiaires).toBeInTheDocument()
    const nombreDactions = within(premiereFeuilleDeRoute).getByText('2 actions attachées à cette feuille de route', {
      selector: 'p',
    })
    ...
    expect(boutonAjouterUneAction).toHaveAttribute(
      'href',
      '/gouvernance/gouvernanceFooId/feuille-de-route/feuilleDeRouteFooId1/action/ajouter'
    )
    ...
    const typeDeLaPremiereAction = within(premiereAction).getByRole('button', {
    ...
    const labelBudgetTotalDeLaFeuilleDeRoute = within(premiereFeuilleDeRoute).getByText(
    ...
    const detailDesMontantsDeSubvention = within(premiereFeuilleDeRoute).getByText(
      'dont 0 € de co-financements et 0 € des financements accordés',
      { selector: 'p' }
    )
    ...
    const titreSectionOuvrirPdf = within(sectionOuvrirPdf).getByRole('heading', {
      level: 2,
    ...
    expect(informationsLink).toHaveAttribute(
      'href',
      '/api/document-feuille-de-route/user/1234/feuille-de-route-fake.pdf'
    )
    ...
    expect(within(secondeFeuilleDeRoute).getByRole('link', { name: 'Voir le détail' })).toHaveAttribute(
      'href',
      '/gouvernance/gouvernanceFooId/feuille-de-route/feuilleDeRouteFooId2'
    )
    ...
    const nombreDActionsSecondeFeuilleDeRoute = within(secondeFeuilleDeRoute).getByText(
      '0 action attachée à cette feuille de route',
      { selector: 'p' }
    )
  ...
  it('quand il n’y a pas de feuille de route, alors un bandeau s’affiche explicitant d’en ajouter', () => {
    ...
    const labelBudgetTotalDesFeuillesDeRoute = screen.queryByText('Budget total des feuilles de route', {
      selector: 'p',
    })
    expect(labelBudgetTotalDesFeuillesDeRoute).not.toBeInTheDocument()

    const contenuFeuilleDeRoute = screen.getByRole('article')
    const contenuTitreFeuilleDeRoute = within(contenuFeuilleDeRoute).getByText('Aucune feuille de route', {
      selector: 'p',
    })
    expect(contenuTitreFeuilleDeRoute).toBeInTheDocument()
    const feuilleDeRoute = within(contenuFeuilleDeRoute).getByText(
  ...
  it('quand il y a plusieurs bénéficiaires, alors le texte est au pluriel', () => {
    ...
    // THEN
    expect(resultat.feuillesDeRoute[0].wordingNombreCofinanceursEtBeneficiaires).toBe(
      '2 bénéficiaires, 3 co-financeurs'
    )
  })
