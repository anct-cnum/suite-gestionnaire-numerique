...
export class PrismaEnveloppesConseillerNumeriqueLoader implements EnveloppesConseillerNumeriqueLoader {
  ...
  // Agrégat régional : consommation et plafonds cumulés sur les départements de la région.
  async getPourDepartements(codes: ReadonlyArray<string>): Promise<EnveloppesConseillerNumeriqueReadModel> {
    try {
      const rows = await prisma.$queryRaw<Array<QueryResult>>`
        WITH agg AS (
          SELECT
          ...
          WHERE a.departement = ANY(${[...codes]})
        ), plafonds AS (
          SELECT de.enveloppe_id, SUM(de.plafond)::int AS plafond
          FROM min.departement_enveloppe de
          WHERE de.departement_code = ANY(${[...codes]})
          GROUP BY de.enveloppe_id
        )
        SELECT
          e.libelle,
          e.date_debut AS "dateDeDebut",
          e.date_fin AS "dateDeFin",
          COALESCE(pl.plafond, 0) AS plafond,
          CASE
            WHEN e.libelle LIKE '%Renouvellement%' THEN agg.total_v2
            WHEN e.libelle LIKE '%Plan France Relance%' THEN agg.total_v1
            ELSE 0
        ...
        FROM min.enveloppe_financement e
        CROSS JOIN agg
        LEFT JOIN plafonds pl ON pl.enveloppe_id = e.id
        -- Type « Conseiller Numérique » : règle canonique classifierTypeEnveloppe (@/shared/enveloppeFinancement)
        WHERE e.libelle LIKE 'Conseiller Numérique%'
        ORDER BY e.libelle
      `
