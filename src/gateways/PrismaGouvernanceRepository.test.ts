...
describe('gouvernance repository', () => {
  ...
  it('ajouter une note privée à une gouvernance', async () => {
    ...
    // WHEN
    await new PrismaGouvernanceRepository().update(
      gouvernanceFactory({
        notePrivee: {
    ...
    // THEN
    const createdRecords = await prisma.gouvernanceRecord.findMany({
      orderBy: { departementCode: 'asc' },
    })
    expect(createdRecords[0]).toStrictEqual(
      gouvernanceRecordFactory({
        departementCode,
    ...
    expect(createdRecords[1]).toStrictEqual(
      gouvernanceRecordFactory({
        departementCode: '93',
  ...
  it('modifier une note privée d’une gouvernance', async () => {
    ...
    // THEN
    const modifiedRecords = await prisma.gouvernanceRecord.findMany({
      orderBy: { departementCode: 'asc' },
    })
    ...
    expect(modifiedRecords[1]).toStrictEqual(
      gouvernanceRecordFactory({
        departementCode: '93',
        editeurNotePriveeId: null,
        // @ts-expect-error
