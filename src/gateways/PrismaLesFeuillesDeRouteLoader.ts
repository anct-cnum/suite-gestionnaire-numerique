...
export class PrismaLesFeuillesDeRouteLoader implements FeuillesDeRouteLoader {
  ...
  #transform(
    ...
    return {
      feuillesDeRoute: feuillesDeRouteRecord.map((feuilleDeRouteRecord, i) => ({
        actions: feuilleDeRouteRecord.action.map((action, j) => {
          const demandeDeSubvention =
            action.demandesDeSubvention[0] as typeof action.demandesDeSubvention[number] | undefined
          const syntheseAction = synthese.feuillesDeRoute[i].actions[j]
          return {
            beneficiaires: demandeDeSubvention?.beneficiaire.map(({ membre }) => fromMembre(toMembre(membre))) ?? [],
        ...
        structureCoPorteuse: feuilleDeRouteRecord.relationMembre
          ...
          : undefined,
        totaux: {
          budget: synthese.feuillesDeRoute[i].budget,
          coFinancement: synthese.feuillesDeRoute[i].coFinancement,
          financementAccorde: synthese.feuillesDeRoute[i].financementAccorde,
        },
