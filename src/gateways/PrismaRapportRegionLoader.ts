...
export class PrismaRapportRegionLoader {
  async get(codeRegion: string): Promise<null | RapportRegionReadModel> {
    try {
      ...
      const [conseillers, beneficiaires, aidants, fneRows] = await Promise.all([
      ...
      const franceNumerique = this.#buildFne(fneRows)
  ...
  #buildFne(rows: Array<FneRow>): RapportRegionReadModel['franceNumerique'] {
    ...
    for (const row of rows) {
      ...
      if (row.beneficiaire_nom !== null) {
        env.beneficiaires.add(row.beneficiaire_nom)
      }
  ...
  async #queryFne(codeRegion: string): Promise<Array<FneRow>> {
    return prisma.$queryRaw<Array<FneRow>>`
      SELECT
        ...
        ds.subvention_demandee,
        benef_st.nom AS beneficiaire_nom,
      ...
      WHERE d.region_code = ${codeRegion}
      ORDER BY d.code, fdr.id, ef.libelle, benef_st.nom
    `
  }
}
