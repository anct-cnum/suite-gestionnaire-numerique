...
function calculateTotauxActions(feuillesDeRoute: FeuillesDeRouteReadModel): FeuillesDeRouteReadModel {
  return {
    ...
    feuillesDeRoute: feuillesDeRoute.feuillesDeRoute.map((feuilleDeRoute) => {
      const { coFinancement, subventionDeLEtat } = feuilleDeRoute.actions.reduce((budget, action) => {
        return {
          coFinancement: budget.coFinancement + action.totaux.coFinancement,
          subventionDeLEtat: budget.subventionDeLEtat + action.totaux.subventionDeLEtat,
        }
      }, { coFinancement: 0, subventionDeLEtat: 0 })

      return {
        ...feuilleDeRoute,
        totaux: {
          budget: coFinancement + subventionDeLEtat,
          coFinancement,
          subventionDeLEtat,
        },
      }
...
function calculateTotauxFeuillesDeRoute(feuillesDeRoute: FeuillesDeRouteReadModel): FeuillesDeRouteReadModel {
  const { coFinancement, subventionDeLEtat } = feuillesDeRoute.feuillesDeRoute.reduce((budget, feuilleDeRoute) => {
    return {
      coFinancement: budget.coFinancement + feuilleDeRoute.totaux.coFinancement,
      subventionDeLEtat: budget.subventionDeLEtat + feuilleDeRoute.totaux.subventionDeLEtat,
    }
  }, { coFinancement: 0, subventionDeLEtat: 0 })
