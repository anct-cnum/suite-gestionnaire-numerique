...
function nombreDeBeneficiaires(
  ...
  const beneficiaires =  actions.flatMap(({ beneficiaireUids }) => beneficiaireUids)
...
function nombreDeCofinanceurs(
  ...
  const cofinanceurs =  actions
    .flatMap(({ coFinancements }) => coFinancements)
    .flatMap(({ coFinanceurUid }) => coFinanceurUid)
...
type ActionReadModel = Readonly<
  {
    ...
    coFinancements: ReadonlyArray<Readonly<{
      coFinanceurUid: string
      montant: number
    }>>
    contexte: string
    description: string
    beneficiaireUids: ReadonlyArray<string>
