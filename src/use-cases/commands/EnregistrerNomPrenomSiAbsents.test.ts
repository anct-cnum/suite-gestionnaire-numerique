import { EnregistrerNomPrenomSiAbsents } from './EnregistrerNomPrenomSiAbsents'
import {
...
describe('enregistrer nom prenom si absents', () => {
  beforeEach(() => {
  ...
  it('le nom et le prénom sont présents : aucune mise à jour n’est effectuée', async () => {
    ...
    // WHEN
    const result = await new EnregistrerNomPrenomSiAbsents(new RepositorySpy()).execute({
      actuels: {
        nom,
        prenom,
      },
  ...
  it.each([
    ...
    async ({ nomAvantCorrection, prenomAvantCorrection }) => {
      ...
      // WHEN
      const result = await new EnregistrerNomPrenomSiAbsents(
        new RepositoryUtilisateurIntrouvableSpy()
      ).execute({
        actuels: {
          nom,
          prenom,
...
class RepositorySpy implements FindUtilisateurRepository, UpdateUtilisateurRepository {
...
class RepositoryUtilisateurIntrouvableSpy
