...
export function pages(nombreDeResultat: number, pageCourante: number, utilisateursParPage: number): Array<number> {
  const nombreDePages = nombreDePage(nombreDeResultat, utilisateursParPage)

  return Array.from({ length: nombreDePages }, (_, index) => index + 1).filter((page): boolean => {
